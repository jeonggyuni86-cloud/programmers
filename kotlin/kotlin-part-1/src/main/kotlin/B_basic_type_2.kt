...
// 코틀린은 변수에 아예 null을 허용하지 않아 NPE문제를 미리 방지할 수 있다.
...
fun main() {
    ...
    // 자료형 뒤에 ?를 붙이면 그때부터 null을 넣을 수 있다.
    val nickname: String? = null
    println("이름 : $name, 별명 : $nickname")

    // 2. 세이프콜(?.) - null이면 실행하지 않고 그냥 null을 반환한다.
    val str1: String? = "Kotlin"
    val str2: String? = null
    println(str1?.length)
    println(str2?.length) // null이나 실행을 건너뛰고 null 반환
    println(str2?.uppercase()?.length)

    // 3. 엘비스 연산자(?:) - null이면 이 기본값을 대신사용
    ...
    // 4. !! : nun null 단언 - null이 아님을 보장
    // 실제로 null이면 그 자리에서 NPE가 발생.
    // -> 꼭 필요한 경우가 아니면 ?. 와 ?:를 사용할것
    ...
    // 자바에서는 equals를 써야하지만, 코틀린은 == 가 곧 equals가 된다
    ...
    // 자바는 작은 타입 -> 큰 타입을 알아서 바꿔주지만(암묵적 형 변환)
    ...
    // 단, 서로 다른 숫자 자료형끼리 '연산'은 가능하다. 결과는 더 큰 자료형이 된다.
