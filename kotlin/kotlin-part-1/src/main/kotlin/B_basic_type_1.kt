...
// * val 와 var
// - val : 최초로 지정한 변수의 값으로 초기화 후 더 이상 값을 바꿀 수 없는 읽기 전용.
// - var : 최초로 지정한 변수위 초깃값이 있더라도 값을 바꿀 수 있다.

// * 자료형 추론
// - 코틀린은 자료형을 저장하지 않을 변수를 선언하면 변수에 할당한 값을 보고 알아서 자료형을 지정할 수 있다.
// 바로 이것을 "자료형을 추론한다" 라고 한다.
// 단, 자료형을 지정하지 않은 변수는 반드시 자료형을 추론할 값을 지정해야 한다.

// * 코틀린은 참조형 자료형을 사용한다.
// 자바에서 기본형과 참조형을 모두 지원하지만 코틀린은 참조형만 사용한다.
...
fun main() {
    ...
    // 5. 자료형을 지정하지 않으면 반드시 초깃값이 있어야 된다.
    // "val z"만 선언 불가 : 컴파일 에러 추론할 값이 없다
    var z: Int
    z = 5  // 나중에 딱 한 번만 할당 가능
    println(z)
    ...
    // 문자열 안에 $를 붙이면 변수의 값을 그 자리에 그대로 끼워넣을 수 있다.
    // 자바처럼 +로 일일이 잉어붙이지 않아도 되고, 읽기도 훨씬 편하다.
    val userName = "홍길동"
    val userAge = 20

    println("이름: $userName, 나이: ${userAge}살")
    // 한글도 "변수명에 쓸 수 있는 글자"라서 $UserAge살 이라고 쓰면
    // 컴파일러가 userAge살 이라는 변수를 찾다가 컴파일 에러를 뿜는다.
    // 만약 변수 바로 뒤에 한글이 붙을때는 반든시 ${변수명} 으로 사용한다.

    // 6-1. ${} 표현식(연산, 함수호출, 프로퍼티 접근)을 넣을때는 중괄호로 감싼다.
    println("내년 나이 : ${userAge + 1}살") // 연산
    println("이름 길이 : ${userName.length}글자") // 포르퍼티 접근
    println("대문자 : ${"kotlin".uppercase()})") // 함수 호출
    println("성인인가? ${userAge >= 19}") // 비교 결과


    // 6-2. $기호 자체를 출력하고 싶을때는 역슬래시로 한다.
    val price = 1000
