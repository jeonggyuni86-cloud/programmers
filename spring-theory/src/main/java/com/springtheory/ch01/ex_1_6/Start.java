package com.springtheory.ch01.ex_1_6;

// * 싱글톤 레지스트리와 오브젝트 스코프
// 애플리케이션 컨텍스트는 싱글톤을 저장하고 관리하는 싱글톤 레지토리 이기도 하다.
// 스프링은 기본적으로 별다른 설정을 하지 않으면 내부적으로 빈 오브젝트를 싱글톤으로 만든다.
...
// * 스프링 빈의 스코프
// 스프링이 관리하는 오브젝트, 즉 빈이 생성되고, 존재하고, 적용되는 범위를 빈의 스코프라 한다.
// 스프링 빈의 기본 스코프는 싱글톤이다.
// 싱글톤 스코프는 컨테이너 내에 한 개의 오브젝트만 만들어서 강제로 제거하지 않는 한, 스프링 컨테이너가 존재하는 동안 계속 유지된다.
// 경우에 따라서는 싱글톤 외의 스코프를 가질 수 있다.
// ex) 웹을 통해 새로운 HTTP 요청이 생길 때마다 생성되는 요청 스코프가 있다.

// 요청이 몰리면 "대기하나?" 에 대한 답 :
// 대기 여부는 빈 스코프가 아니라 스레드(일꾼)가 결정한다. (스레드는 무한하지 않다)
...
import com.springtheory.ch01.ex_1_6.dao.DaoFactory;
import com.springtheory.ch01.ex_1_6.dao.UserDAO;
...
public class Start {
    static void main(String[] args) {
        ...
        var userDao1 = factory.userDAO();
        var userDao2 = factory.userDAO();

        System.out.println("userDao1: " + userDao1);
        System.out.println("userDao2: " + userDao2);

        var context = new AnnotationConfigApplicationContext(DaoFactory.class);
        var userDao3 = context.getBean(UserDAO.class);
        var userDao4 = context.getBean(UserDAO.class);
