package com.springtheory.ch01.ex_1_5;


// * 스프링의 제어의 역전(IoC , Inversion of Control)
// 이제 DaoFactory를 스프링에서 사용이 가능하도록 변신시켜보자.
// 스프링에서는 스프링이 제어권을 가지고 있고, 직접 만들고, 관계를 부여하는 오브젝트를 빈(Bean)이라 한다
// ex) UserDao, MessageDao ...
// 자바빈은 오브적트 단위의 애플리케이션 컴포넌트를 의미히고,
// 동시에 스프링 빈은 컨테이너가 생성, 관계설정, 사용 등을 제어해주는 제어의 역전이 적용된 오브젝트이다.
// 스프링에서는 빈의 생성과 관계설정 같은 제어를 담당하는 IoC 오브잭트를 '빈 팩토리'라 한다
// 정확히는 '빈 팩토리' 보다는 이를 좀 더 확장한 '애플리케이션 컨텍스트'를 주로 사용한다.
// '애플리케이션 컨텍스트'는 IoC 방식을 따라 만들어진 일종의 '빈 팩토리' 라 생각하면 된다.
...
// * 컨테이너
// 컨테이너란 이 컴포넌트들을 담아두고, 생성하고, 서로 연결
// 생명 주긱 까지 관리해주는 실행 환경을 말한다.
// 컴포넌트가 '부품'이라면 컨테이너는 그 부품들을 조립하고, 가동시키는 '공장'이라 생각할 수 있다.
...
import com.springtheory.ch01.ex_1_5.dao.DaoFactory;
import com.springtheory.ch01.ex_1_5.dao.UserDAO;
