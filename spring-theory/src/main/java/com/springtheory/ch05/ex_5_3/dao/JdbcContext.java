package com.springtheory.ch05.ex_5_3.dao;
...
// * 트랜젝션에 '참여' 할 수 있게 바꾼 HdbcContext
// ex_5_1 : 매번 새 커넥션을 만들고 try with resources로 바로 닫았다.
// -> 메서드 마다 커넥션이 다르므로 여러 DAO 호출을 '하나의 트렌젝션'으로 묶을 수 없다.
...
public class JdbcContext {
    ...
    public void workWithStatementStrategy(StatementStrategy statementStrategy) throws SQLException {
        var conn = DataSourceUtils.getConnection(this.dataSource);
        try (
    ...
    public <T> List<T> query(StatementStrategy strategy, RowMapper<T> rowMapper) throws SQLException, ClassNotFoundException {
        ...
        try(    var pStmt = strategy.makeStatement(conn);
