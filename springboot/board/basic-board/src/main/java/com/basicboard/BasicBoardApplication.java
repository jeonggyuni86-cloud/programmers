...
// - 이를 DB같은 영구 저장소에 저장해 다시 꺼내쓸 수 있게 만드는것이 영속성이다.
// - 이 "객체 <-> DB" 변환을 담당하는 계층을 영속성 계층이라 하며, domain.repository가 그 역할이다.

// - 관련 용어
// 영속화 : 객체를 DB에 저장하는 행위
// 영속성 컨텍스트 : JPA가 엔티티를 관리하는 1차 게시공간, 엔티티와 상태 변화를 추적해 자동으로 SQL을 실행한다.

// * ORM(Object Relational Mapping, 객체-관계 매핑)
// - 자바의 '객체(Object)'와 관계형 DB의 '테이블(Relation)'을 서로 연결해주는 기술이다.
...
// - 이 어노테이션이 있어야 스프링이 시작할 때 @Scheduled가 붙은 메서드를 찾아 스케줄러에 등록한다
@EnableScheduling
