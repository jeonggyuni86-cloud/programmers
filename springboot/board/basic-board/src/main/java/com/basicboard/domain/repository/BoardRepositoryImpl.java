...
public class BoardRepositoryImpl implements BoardRepositoryCustom {
    ...
    public Page<BoardListItemResponseDto> searchBoards(BoardSearchRequestDto condition, Pageable pageable) {
        ...
        // - 예) 마지막 페이지가 아니고 결과가 페이지크기보다 작으면 굳이 count 쿼리를 하지 않는다
        // - countQuery::fetchOne을 "지금 실행"이 아니라 필요하면 "실행할 함수"로 넘긴다.

        //  countQuery::fetchOne == () -> countQuery.fetchOne()
    ...
    // - N + 1
    // 1 = 처음 의도하고 날린 쿼리 1번
    // N = 그 결과 행 수만큼 "추가로"나가는 쿼리 N번(게시글 마다 댓글 조회 1번씩)

    // # 실제 SQL 흐름
    // List<Board> boards = boardRepository.findAll();
    // for (Board board : boards) {
    //      board.getComments(); // 순회하며 LAZY가 꺠어날 때마다..
    // }
    ...
    // * goe / loe 는 비교 연산자의 약어이다.
    // - gt(Greater Than, >)
    // - goe(Greater Than or Equal, >=)
    // - lt(Less than, <)
    // - loe(Less than or Equal, <=)
    // -> 아래 goe + loe 한 쌍이 from 이상 to 이하를 찾게 된다.
    ...
    // - JPAQueryFactory : EntityManager를 품은 "본 쿼리" 생성기. fetch() / fetchOne()으로 SQL을 실행할 수 있다.
    // - JPAExpressions : static 유틸 "서브쿼리 표현식(조각)"만 만들고, 실행능력은 없다
