...
// # 전제 조건 2가지
// - 메인 클래스에 @EnableScheduling 이 있어야 한다.
// - 이 클래스가 스프링 빈 이어야 한다(@Component)

// @Scheduled 메서드의 형태 : 파라미터 없음 + 반환 void
// - 정해진 시각에 "스프링이" 호출하는 것이라, 인자를 넘겨줄, 반환을 받을 주체도 없다.
...
public class BoardBatchScheduler {
    ...
    // - zone 옵션 - 시작의 기준 시간대를 정한다.
    // - 지정하지 않으면 "서버(JVM)의 시간대"를 따른다. -> 해외 클라우드 서버(UTC)에 배포하면,
    // "9시에 돌아라"가 한국 시간 10시에 도는 사고가 난다.
    ...
    // initialDelay = 10_000 : 10초, 앱이 뜨고 10초 뒤 첫 실행
    // fixedDelay = 3_600_000 : 1시간 : 그 뒤로는 "이전 실행이 끝난 시점" 부터 1시간 간격으로 반복
    ...
    public void reportOrphanFiles() {
        ...
        Set<String> ref = boardRepository.findAll().stream()
        ...
        for(File file : files) {
            ...
            checked++;
            if(!ref.contains(file.getName())) {
                orphanCount++;
                log.warn("[고아 파일 발견] 어느 게시글도 참조하지 않음 : {}", file.getName());
            }
        }
        log.info("[고아 파일 점검 완료] 전체 {}개 중 고아 {}개", checked, orphanCount);
    }
}
