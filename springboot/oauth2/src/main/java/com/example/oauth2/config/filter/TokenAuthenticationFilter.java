...
// 이 필터는 "요청에 실려온 토큰"에서 인증상태를 복원한다.
...
// 4) 토큰이 없거나 무효면 인증 없이 통과 -> 보호된 경로라면 체인 끝의 AuthorizationFilter 가 거부한다.
    ...
    protected void doFilterInternal(
        ...
        if(token != null){
            ...
            if(status == TokenStatus.VALID) {
                ...
                // * principal 이란
                // Authentication 객체는 3가지로 구성된다.
                // - principal   : "인증된 주체가 누구인가" - 사용자를 대표하는 객체 (보통 UserDetails)
                // - credential  : "인증에 사용된 증명 수단" (비밀번호, 토큰 등, 인증 후에 보통 지운다)
                ...
                // 2) api/refresh-token(access 만료) <- 401
                log.warn("{}, Token has expired", requestURI);
