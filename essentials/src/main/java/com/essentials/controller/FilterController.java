...
// 그래서 이 서블릿을 '프론트 컨트롤러'라고 부른다.
// 개발자가 만들지 않아도 스프링부트가 자동 구성으로 미리 등록해준다.

// 요청 하나가 들어와 응답이 나가기 까지 흐름
...
// 이때 파라미터(@RequestParam, HttpSession 등) 알맞게 만들어 넣어준다
...
// 둘 다 웹 요청을 받아 처리하는 컨트롤러 이지만, 메서드가 반환하는 String을
// '어떻게' 해석하는지가 다르다

// - @Controller: 반환하는 String을 'view의 이름 (즉 html 파일 이름)'으로 본다.
// SessionController, CookieController 처럼 HTML 페이지를 보여줄 때 사용한다.

// - @RestController: 반환하는 String(또는 객체)을 '데이터 그 자체'로 본다.
// return "Hello World!"는 뷰를 찾지 않고, 그 글자 그대로 응답 본문에 보여줄 때 사용한다.
// 객체를 반환하면 JSON으로 변환해준다. 그래서 REST API를 만들때 사용한다.
...
// 요청이 Dispatcher Servlet에 '도착하기도 전에' 먼저 거치는 관문이 필터다.
// 서블릿컨테이너(톰캣) 수준에서 동작하며, 스프릴 MVC의 바깥에 위치한다.
// 즉, 필터는 '요청이 들어올 떄', '응답이 나갈 때' 모두 가로챌 수 있다.
//   [하는 일]
